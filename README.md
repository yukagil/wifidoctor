...
利用者が ［いまの状況をコピー］ を押すと、次の内容が貼られてくる。
チャットで状況を聞いたときの返答が、聞き返し無しで揃うようにしてある。

```
【Wi-Fi の状況】2026-09-03 14:01:54 (Asia/Tokyo)
端末: Mac15,6 / macOS 26.2 / Wi-Fi MAC 80:a9:97:xx:xx:xx
接続: SSID "OFFICE" / AP 会議室A (aa:bb:cc:dd:ee:01) / ch44 (5GHz 40MHz) / RSSI -56dBm / リンク 400Mbps 11ax
状態: APが混雑（62点） / このAPに接続して 24分

■ いまの計測値
  端末→AP   応答 18.2ms / ゆらぎ 6.1ms / とりこぼし 0.4% / 同じチャンネル付近に他 7台
  AP以降    外部への応答 21ms / 名前解決 15ms
  このMac   CPU 12% / 端末側の負荷は原因ではありません

■ 直近60分
  観測 58分 / うち不調 22分（38%）
  13:01:57-13:26:46（24分）  APが混雑  最悪32点  RSSI -59dBm / ch44 / 309Mbps / GW 42.0ms
...
引き当てられない**ため。何が含まれるかは文面の末尾に明記してあり、
利用者は中身を見た上で貼ることになる。

より長い1日分が要るときは ［レポートを書き出す］ の .txt を頼むこと。
...
- **［いまの状況をコピー］** … パネル → 設定。情シスに「どういう状況？」と
  聞かれたときの返信を作って貼り付け板に置く。チャットにそのまま貼れる長さ
  （20行前後）で、端末・接続先・実測値・直近1時間の不調・確認してほしいこと、が入る
