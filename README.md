...
```
Wi-Fi 診断レポート  今日  [WiFiDoctor 1.0 (23 9dcc90d)]
端末: Mac15,6 / macOS 26.2 / Wi-Fi MAC 80:a9:97:xx:xx:xx
観測 2241 件 / Asia/Tokyo
...
■ いまの状態
  ...
  判定 CONGESTED（APが混雑） / スコア 62 / 同一BSSIDに 24分 接続中
  第一ホップ(端末→AP)  GW 192.168.100.1 RTT 18.2ms / ジッタ 6.1ms / ロス 0.4%
  上流(AP以降)         RTT 21ms / DNS 15ms
  周辺                 同一ch±4に他 7台 / この地点で実用可能なAP 2台
  端末側               CPU 12% / 端末の送受信 2.3Mbps / 端末側の負荷は判定に影響なし

■ 判定ごとの時間
  OK（良好）                           100分
  CONGESTED（APが混雑）                 73分
  STICKY（遠いAPを掴んでいる）          10分

■ 不調区間（30秒以上）
  9/3 11:33:47-9/3 11:37:54（1分29秒）  CONGESTED（APが混雑）  最悪スコア50
      AP 44:12:44:b1:41:70（会議室A） / ch44 / RSSI -52dBm / リンクレート 516Mbps
        / GW RTT 73.0ms / ジッタ 92.9ms

■ AP別の実績
  会議室A
    44:12:44:b1:41:70 / 接続 195分 / スコア p50 85, p10 57 / RSSI 中央 -60dBm
    GW RTT p50 10ms, p95 42ms / ジッタ p50 4ms, p95 30ms / ロス 0.2%

■ 情シスへの申し送り
  ・RSSI は良好なのに、第一ホップの RTT が伸びる時間が長い（CONGESTED）
...
判定は内部の符号（`CONGESTED` など）と日本語を併記している。符号だけでは
このアプリを知らない人に伝わらず、日本語だけでは他の資料と突き合わせられないため。
